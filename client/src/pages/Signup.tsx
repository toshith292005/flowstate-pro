...
import { User, Mail, Lock, ArrowRight, ArrowLeft, AlertCircle, Eye, EyeOff, Loader2, Zap, Sun, Moon } from "lucide-react";
...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ This URL is correct (Standard Signup)
    ...
    <div className="min-h-[100dvh] w-full font-sans selection:bg-indigo-500 selection:text-white flex items-center justify-center p-4 md:p-6 relative bg-slate-50 dark:bg-black overflow-hidden transition-colors duration-300">
      ...
      <div className="w-full max-w-md bg-white dark:bg-white/5 backdrop-blur-xl rounded-3xl shadow-xl dark:shadow-2xl p-6 md:p-10 space-y-6 border border-slate-200 dark:border-white/10 relative z-10">
        ...
        <div className="text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base mt-2">Join FlowState to boost your productivity</p>
        </div>

        {/* 🚀 CRITICAL FIX: Added '/api' here as well */}
        <a 
        ...
        <div className="relative flex items-center py-1">
          ...
          <span className="flex-shrink-0 mx-4 text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider">Or register with email</span>
          <div className="flex-grow border-t border-slate-200 dark:border-white/10"></div>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          <div className="space-y-2">
            <label htmlFor="name" className="text-xs md:text-sm font-semibold text-slate-600 dark:text-slate-300 ml-1">Full Name</label>
            <div className="relative group">
              <User className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors" size={20} />
              <input 
                id="name"
                name="name"
                autoComplete="name"
          ...
          <div className="space-y-2">
            <label htmlFor="password" className="text-xs md:text-sm font-semibold text-slate-600 dark:text-slate-300 ml-1">Password</label>
            <div className="relative group">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold h-12 rounded-xl transition-all shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
          >
        ...
        <div className="space-y-4 text-center pt-2">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Already have an account? <Link to="/login" className="text-indigo-600 dark:text-indigo-400 font-semibold hover:text-indigo-500 dark:hover:text-indigo-300 hover:underline p-1">Sign In</Link>
          </p>
