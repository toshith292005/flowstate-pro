...
export default function Calendar() {
  ...
  // MOBILE STRIP LOGIC (Shows 14 days around today)
  ...
  return (
    <div className="p-4 md:p-10 pb-32 space-y-6 max-w-7xl mx-auto h-screen flex flex-col overflow-hidden bg-black">
      ...
      <div className="flex items-center justify-between shrink-0">
        ...
        {/* Navigation - Hidden on mobile because mobile uses the scrollable strip */}
      ...
      {/* MOBILE DATE STRIP (Visible only on Mobile) */}
        ...
        {mobileDays.map((day, idx) => {
          ...
          return (
            ...
            >
              ...
              {hasTasks && !isSelected && <div className="absolute bottom-2 w-1 h-1 bg-indigo-400 rounded-full"></div>}
            </button>
          );
        })}
      </div>
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {tasks.slice(0, 2).map((t: any) => (
                        <div key={t._id} className="h-1.5 w-full bg-indigo-500/50 rounded-full"></div>
                      ))}
                    </div>
                  </div>
                );
          ...
          <div className="w-full lg:w-96 flex flex-col h-full min-h-0">
            <div className="mb-4 flex items-center justify-between shrink-0 px-1">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                ...
                {isSameDay(selectedDate, new Date()) ? "Today's Schedule" : format(selectedDate, "MMMM do")}
              </h3>
                ...
                selectedTasks.map((task: any) => (
                  <div key={task._id} className="p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-indigo-500/50 transition-all">
                    ...
                    <div className="mt-3 flex items-center justify-between text-[11px] text-slate-500">
                      ...
                      <span className="bg-white/10 px-2 py-0.5 rounded uppercase font-bold">{task.status}</span>
                    </div>
                  </div>
                ))
              ) : (
